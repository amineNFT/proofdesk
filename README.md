...
Approval requires every claim to be supported and every acceptance criterion to be met. An explicit contradiction or omitted criterion requires revision. Unavailable, ambiguous, or unquoted evidence leaves the result inconclusive; a quoted passage is matched against the retrieved source ignoring case and punctuation, so one run's formatting cannot turn real evidence into missing evidence, while an invented passage is still rejected. Validators independently re-run the assessment — their own source fetch and their own model calls — and compare their result with the leader's under a custom Equivalence Principle: a contradicted claim or an unmet criterion must match exactly, because those decide who may be paid, while a validator that could only reach "inconclusive" does not overturn an approval. Requiring identical per-claim verdicts instead made rounds end Undetermined whenever a passage was paraphrased, which wrote no review at all.
